...
class CalendarVacation < ActiveRecord::Base
  ...
  scope :events_for_dates, lambda { |calendar, b_date, e_date| {:conditions => ["calendar_id = ? AND date_holiday >= ? AND date_holiday <= ?", calendar, b_date, e_date]} }
  scope :events_for_free_dates, lambda { |calendar, b_date, e_date| {:conditions => ["calendar_id = ? AND date_holiday >= ? AND date_holiday <= ? AND fixed_date = 0", calendar, b_date, e_date]} }
  scope :event_for_date, lambda { |calendar, date| {:conditions => ["calendar_id = ? AND date_holiday = ? ", calendar, date]} }
  scope :event_fixed_date_for_year, lambda { |calendar, year| {:conditions => ["calendar_id = ? AND date_holiday <= ?  AND fixed_date = ?", calendar, Date.new(year, 12, 31), 1]} }
